...
@export var distancia_deteccion = -600.0
...
func _on_frame_changed():
	...
	if $AnimatedSprite2D.animation == "ataque" and $AnimatedSprite2D.frame == 3:
		if esperando_lanzar:
...
func crear_y_lanzar_bomba():
	...
	bomba.global_position = global_position + Vector2(0, -135)
